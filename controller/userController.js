...
export const updateUserLocationController = async (req, res) => {
  try {
    const { id } = req.params;
    const {  longitude , latitude, online } = req.body;
  const updateFields = {
      ...(longitude && { longitude }),
      ...(latitude && { latitude }),
      ...(online && { online }),
...
export const getAllBlogsController = async (req, res) => {
  try {
    ...
    if (cachedBlogs) {
      return res.status(200).send({
        message: "All Blogs List (from cache)",
        BlogCount: cachedBlogs.length,
        success: true,
        blogs: cachedBlogs,
      });
    }
    ...
    return res.status(200).send({
      message: "All Blogs List (from DB)",
...
export const UsergetAllProducts = async (req, res) => {
  ...
  try {
    const filter = {
      ...
      // Step 1: get product by product id
    if (pro && mongoose.Types.ObjectId.isValid(pro)) {
      const product = await productModel
        .findById(pro)
        .select("Category");

      if (!product || !product.Category.length) {
        return res.status(404).json({
          success: false,
          message: "Product or categories not found",
        });
      }

      // Step 2: find products having ANY of these categories
      filter.Category = {
        $in: product.Category,
      };
    ...
    const products = await productModel.find(
      filter,
      "_id title slug regularPrice salePrice gst pImage"
    );
 ...
 export const UsergetAllHomeProducts = async (req, res) => {
  try {
    ...
    if (cachedProducts) {
      return res.status(200).send({
        message: "All products List (from cache)",
        proCount: cachedProducts.length,
        success: true,
        products: cachedProducts,
      });
    }

    // 🔹 2️⃣ Fetch from DB (use lean for speed)
    ...
    // 🔹 3️⃣ Save to cache for 1 hour
    cache.set(cacheKey, products);

    return res.status(200).send({
      message: "All products List (from DB)",
      proCount: products.length,
      success: true,
      products,
    });
    ...
    return res.status(500).send({
      message: `error while All products ${error.message}`,
      success: false,
      error,
    });
  }
...
// get home data
 ...
 export const getHomeData = async (req, res) => {
  try {
    const cacheKey = "home_settings";

    // 🔹 1️⃣ Check cache first
    const cachedHomeData = cache.get(cacheKey);
    if (cachedHomeData) {
      return res.status(200).json({
        message: "Found home settings! (from cache)",
        success: true,
    ...
    return res.status(200).json({
      message: "Found home settings! (from DB)",
      success: true,
      homeData,
    });
  } catch (error) {
    return res.status(400).json({
      message: `Error while getting home settings: ${error.message}`,
      success: false,
      error,
    });
  }
  ...
  console.log('OTP',OTP)
    ...
    if (mode === "COD") {
      ...
      //  await sendOrderOTP(phone, norder_id);
        await sendMaidNotification(norder_id);
      ...
      const cancel_url = process.env.CANCEL_URL;
        ...
        await sendMaidNotification(order_id);
...
async function sendOrderConfirmationEmail(email, username, userId, newOrder) {
  try {
    ...
    <thead>
      ...
      <img width="200" src="${'https://backend-olxs.onrender.com/uploads/new/image-1755174201972.webp'}" />
 </th>
...
export const EmailVerify_new = async (req, res) => {
  try {
    const { email } = req.body;
    ...
    const secureBool = String(process.env.MAIL_SECURE).toLowerCase() === "true";

    const transporter = nodemailer.createTransport({
      host: process.env.MAIL_HOST,                 // smtp.hostinger.com
      port: Number(process.env.MAIL_PORT),         // 465
      secure: secureBool,                          // true for 465
      auth: {
        user: process.env.MAIL_USERNAME,           // mail@webplusera.com
        pass: process.env.MAIL_PASSWORD,
      },
      // reliability & diagnostics
      family: 4,                    // force IPv4 (avoids IPv6 hangs)
    ...
    const info = await transporter.sendMail({
      ...
      // Ensure envelope MAIL FROM also matches the authenticated mailbox to avoid 553
      envelope: { from: process.env.MAIL_USERNAME, to: email },
      // If you want replies to go to info@ even though you send from mail@
      // replyTo: "info@webplusera.com",
    });
...
export const GetAllCategoriesBySlugController_no = async (req, res) => {
  try {
...
export const GetAllCategoriesController = async (req, res) => {
  try {
    const { parentSlug } = req.params;
    const { filter, price, page = 1, perPage = 10, location } = req.query;
    ...
    /* ===========================
       CASE 1️⃣ : OLD FUNCTIONALITY
       =========================== */
    if (parentSlug) {
      MainCat = await categoryModel
        .findOne({ slug: parentSlug, status: "true" })
        .select(
          "title metaTitle metaDescription metaKeywords image description specifications slide_head slide_para filter slug canonical"
        )
      ...
      const collectIds = (cats, ids = []) => {
        for (const c of cats) {
          ids.push(c._id);
          if (c.subcategories?.length) {
            collectIds(c.subcategories, ids);
          }
        }
        return ids;
      };

      categoryIds = collectIds(categories, [parentId]);
    }

    /* ===========================
       CASE 2️⃣ : /all/category
       =========================== */
    if (!parentSlug) {
      const rootCategories = await categoryModel
        .find({ parent: null, status: "true" })
        .lean();

      for (const cat of rootCategories) {
        cat.subcategories = await getAllCategoriesByParentId(cat._id);
      }
      ...
      categoryIds = collectIds(categories);
    }
    ...
    if (filter) {
      ...
      const variationFilters = Object.entries(filterParams).map(
        ([name, value]) => ({
          $elemMatch: {
            name,
            value: { $in: value.split(",").map(v => v.trim()) },
          },
        })
      );
      filters.variations = { $all: variationFilters };
    ...
    console.error("GetAllCategoriesBySlugController:", error);
    return res.status(500).json({
      success: false,
      message: "Internal Server Error",
    });
...
export const userOrdersController = async (req, res) => {
  try {
    const userOrder = await userModel.findById(req.params.id).populate({
      path: "orders",
      select: "_id createdAt totalAmount status mode orderId OTP payment", // Select only _id and title fields
      options: {
...
export const userOrdersViewController = async (req, res) => {
  ...
  path: "orders",
  match: { _id: orderId }, // filter specific order
  populate: {
    path: "agentId",
    select: "username email longitude latitude phone"
  }
});
...
export const AddRating = async (req, res) => {
  try {
    const { userId, rating, comment, productId,username } = req.body;
    ...
    // 2️⃣ Update username in userModel
    user.username = username;
    await user.save(); // ✅ This saves the updated username in the database
...
export const ViewProductRating_no = async (req, res) => {
  try {
...
export const ViewProductRating = async (req, res) => {
  try {
    ...
    const cacheKey = `product_ratings:${productId}`;

    // 🔹 1️⃣ Check cache
    const cachedRatings = cache.get(cacheKey);
    if (cachedRatings) {
      return res.status(200).json({
        success: true,
        source: "cache",
        message: "Getting product ratings successfully!",
        productRatings: cachedRatings,
      });
    }

    // 🔹 2️⃣ Fetch from DB (optimized)
    const ratings = await ratingModel
      .find({ productId, status: 1 })
      .populate("userId", "username")
      .select("rating comment createdAt userId")
      .lean();

    const ratingsWithUserDetails = ratings.map((r) => ({
      rating: r.rating,
      comment: r.comment,
      username: r.userId?.username || "Unknown",
      createdAt: r.createdAt,
      userId: r.userId?._id || "Unknown",
    }));

    // 🔹 3️⃣ Save to cache for 1 hour
    cache.set(cacheKey, ratingsWithUserDetails, 3600);
...
export const getProductIdUserBySlug_no  = async (req, res) => {
  try {
...
export const getProductIdUserBySlug = async (req, res) => {
  try {
    ...
    if (cachedProduct) {
      return res.status(200).json({
        message: "Fetch single product (from cache)",
        success: true,
        source: "cache",
    ...
    return res.status(200).json({
      message: "Fetch single product (from DB)",
      success: true,
      source: "db",
      Product,
    });
...
// };
 ...
 export const ViewAllZonesOnly = async (req, res) => {
  try {
    const cacheKey = "all_zones_only";

    // 🔹 1️⃣ Check cache first
    const cachedZones = cache.get(cacheKey);
    if (cachedZones) {
      return res.status(200).json({
        success: true,
        source: "cache",
        uniqueLocations: cachedZones,
      });
    }
    ...
    const products = await productModel
      .find()
      .populate("userId", "coverage username email phone")
      .lean();

    // 🔹 3️⃣ Collect & flatten coverage arrays
    const allLocations = products
      .map((product) => product.userId?.coverage || [])
      .flat();

    // 🔹 4️⃣ Remove duplicates & undefined
    const uniqueLocations = [...new Set(allLocations)].filter(Boolean);

    // 🔹 5️⃣ Save to cache for 1 hour
    cache.set(cacheKey, uniqueLocations);
...
const sendLogOTP_serverdown = async (phone, otp) => {
  try {
    // Construct the request URL with query parameters
    const queryParams = querystring.stringify({
      username: "helply.trans",
      password: "9xvkG",
      unicode: false,
      from: "HELPLY",
      to: phone,
  ...
  return new Promise((resolve) => {
    ...
    const req = https.get(url, { timeout: 8000 }, (res) => {
      ...
      res.on("end", () => {
        ...
        return resolve({
          ok: false,
          error: `SMS gateway responded with ${res.statusCode}`,
          body,
        });
      });
    });
...
const sendLogOTP_old = (phone, otp) => {
  return new Promise((resolve) => {
    const queryParams = querystring.stringify({
      username: "helply.trans",
      password: "9xvkG",
...
const sendMaidNotification = async (orderId) => {
  try {
     if (!orderId) {
      return res.status(400).send("Missing required field: orderId");
    }

    // 1) Get all maid users who have fcmToken
    const maidUsers = await userModel.find({
      type: 1,
      empType: 2,
    ...
    if (maidUsers.length === 0) {
      console.log("No maid users with FCM token found");
      return;
      // return res.status(404).send("No maid users with FCM token found");
    }

    // 2) Collect all tokens
    const tokens = maidUsers
      .map((u) => u.fcm)
      .filter(Boolean);

    // 3) Build base message (token is added in sendEachForMulticast)
    const message = {
      notification: {
        title: "New Order Update",
        body: `Tap to view details of Order #${orderId}`,
      },
      data: {
        orderId: orderId.toString(),
      },
      tokens, // pass the array of tokens here
    };

    // 4) Use multicast to send to multiple devices
    const response = await admin.messaging().sendEachForMulticast(message);

    console.log("Notification sent summary:", response);
    // return res.status(200).json({
...
export const AuthUserByID = async (req, res) => {
  try {
    ...
    // Get orders where agentId matches and startTime is set but endTime is not
...
const myorder = await orderModel.find({
  userId: id,
  status: { $nin: [0, 7] },
}).select('OTP orderId')  // Specify fields to include
.sort({ _id: -1 }); // -1 for descending order
    ...
    return res.status(200).json({
      ...
      activeOrders: orders, // orders with startTime set and endTime null
    });
  } catch (error) {
    return res.status(500).send({
      message: `Error on Auth: ${error.message}`,
      success: false,
      error,
    });
  }
...
export const completeOrderUser = async (req, res) => {
  try {
    const { id } = req.params;
    const { otp } = req.body;
...
const finalPrice = amount + gstAmount;

    const wallet = Order.totalAmount - finalPrice;
    const currentWallet = Number(user.wallet); // Ensure user.wallet is a number
    const amountToAdd = Number(wallet); // Ensure wallet is a number
...
const generateUserInvoicePDF_old = async (invoiceData) => {
...
export const downloadUserInvoice_old = async (req, res) => {
  try {
...
export const downloadWalletInvoice = async (req, res) => {
  try {
    const { invoiceId } = req.body; // Assuming invoiceData is sent in the request body
    if (!invoiceId) {
      return res.status(400).send("Invoice ID is required");
    ...
    const pdfBuffer = await generateUserInvoicePDFWallet(invoiceData);
...
export const downloadVendorInvoice = async (req, res) => {
  try {
    const { invoiceId,userId } = req.body; // Assuming invoiceData is sent in the request body
    if (!invoiceId) {
      return res.status(400).send("Invoice ID is required");
    }
    // Fetch invoice data from the database
    ...
    const pdfBuffer = await generateVendorInvoicePDF(invoiceData,userId);
...
export const downloadUserOrderInvoice = async (req, res) => {
  try {
    const { invoiceId } = req.body; // Assuming invoiceData is sent in the request body
    if (!invoiceId) {
      return res.status(400).send("Invoice ID is required");
    ...
    const pdfBuffer = await generateUserOrderInvoicePDF(invoiceData);
...
export const downloadVendorOrderInvoice = async (req, res) => {
  try {
    const { invoiceId } = req.body; // Assuming invoiceData is sent in the request body
    if (!invoiceId) {
      return res.status(400).send("Invoice ID is required");
    ...
    const pdfBuffer = await generateVendorOrderInvoicePDF(invoiceData);
...
const generateUserOrderInvoicePDF = async (invoiceData) => {
  console.log(invoiceData);
  ...
  const formatTime = (dateString) => {
    ...
    const home = await homeModel.find();
  	...
  	const logo = home[0].meta_logo;

  let local = 0;
  if(invoiceData.Local){
    local = invoiceData.Local;
  }
  // Define the HTML content
  const htmlContent = `
    <div class="invoice">
...
<div style="margin-bottom: 15px;margin-top: 15px;border-top-style: solid;border-top-width: 3pt;border-top-color: #4F81BC;"> </div>
        ...
        <div class="invoice-header-left">
           ...
           <p> <b> Contact Person: </b> ${invoiceData.details[0]?.username}</p>
            <p> <b> Email Id: </b> ${invoiceData.details[0]?.email}</p>
            <p> <b>  Phone No.: </b> ${invoiceData.details[0]?.phone}</p>
      ...
      if (invoiceData.payment === 0) {
        ...
        <div class="invoice-header-right">
          <h2 style="margin-top:0px;"> Customer Copy </h2>
            <p> <b> Customer Copy No.:</b> #${invoiceData?.orderId}</p>
          <p> <b>  Customer Copy Date:</b> ${formatDate(
      invoiceData?.createdAt
    )}     </p>

       <p>   <b>   State Name :</b> ${invoiceData.details[0]?.state} </p>
    ...
    width: 50%;
" > </p>
              Order Id : <b> ${invoiceData.orderId} </b>
           <br>
               </td>
          ...
          <b>  Sub Total </b>
            ...
            <tr>
              ...
              We declare that this customer copy shows the actual price of the all the services & gross payable amt described <br>and that all particulars are true and correct.
<br><br>
...
const generateVendorOrderInvoicePDF = async (invoiceData) => {
  ...
  const home = await homeModel.find();
   const commission = home[0].commission;
    const meta_logo = home[0].meta_logo;

     
const totalamt = invoiceData.totalAmount;
const totalWithGST = Number((totalamt * (commission / 100)).toFixed(2));
const amountWithoutGST = Number((totalWithGST / (1 + gstRate)).toFixed(2));
const GSTAMT = Number(totalWithGST * gstRate);
...
<div style="margin-bottom: 15px;margin-top: 15px;border-top-style: solid;border-top-width: 3pt;border-top-color: #4F81BC;"> </div>
        ...
        <div class="invoice-header-left">
           ...
           <p> <b> Contact Person: </b> ${invoiceData.agentId?.username}</p>
            <p> <b> Email Id: </b> ${invoiceData.agentId?.email}</p>
            <p> <b>  Phone No.: </b> ${invoiceData.agentId?.phone}</p>
      ...
      if (invoiceData.payment === 0) {
        ...
        <div class="invoice-header-right">
          <h2 style="margin-top:0px;">TAX INVOICE</h2>
            <p> <b> Orrder No.:</b> #${invoiceData?.orderId}</p>
      ...
      <table class="invoice-table">
        ...
        <tbody>
            ...
            <tr   >
              <td style="text-align: left;" > <b> Convenience Fees ( IT Service Fees ) </b>
                <p style="
    border-top: 1px dashed grey;
    width: 50%;
" > </p>
           ...
           <br>
              ...
              <td style="text-align: right;" colspan="2"  >     
              <b>  Rs. ${(totalWithGST ).toFixed(2)} </b> </td>
            </tr>
...
<tr>
              ...
              <b> TOTAL </b> </td>
               ...
               <td style="text-align: right;"  > 
              <b> Rs. ${(totalWithGST + GSTAMT).toFixed(2) } </b> </td>
  ...
  const htmlContent = `
    <div class="invoice">
      <div class="invoice-header">
        <div class="invoice-header-left" style="flex:none;">
          <img 
src="https://backend-olxs.onrender.com/uploads/new/image-1755174201972.webp"
  alt="Company Logo" width="200">
...
<div style="margin-bottom: 15px;margin-top: 15px;border-top-style: solid;border-top-width: 3pt;border-top-color: #4F81BC;"> </div>
  ...
  ${invoiceData.userId?.gstin && invoiceData.userId?.gstin.length !== 0
      ...
      if (invoiceData.payment === 0) {
        ...
        <div class="invoice-header-right">
          <h2 style="margin-top:0px;"> Customer Copy </h2>
            <p> <b> Customer Copy No.:</b> #${invoiceData?.paymentId}</p>
          <p> <b>  Customer Copy Date:</b> ${formatDate(
      invoiceData?.createdAt
    )}     </p>
...
const generateUserInvoicePDFWallet = async (invoiceData) => {
 console.log(invoiceData);
  ...
  const htmlContent = `
    <div class="invoice">
      <div class="invoice-header">
        <div class="invoice-header-left" style="flex:none;">
          <img 
src="https://backend-olxs.onrender.com/uploads/new/image-1750835801961.webp"
  alt="Company Logo" width="200">
...
" > </p>
           ...
           <br>
              ...
              <td style="text-align: right;" colspan="2"  >     
              <b>  Rs.${parseFloat(invoiceData?.totalAmount.toFixed(2))} </b> </td>
            </tr>
...
const generateVendorInvoicePDF = async (invoiceData,userData) => {
...
<div style="margin-bottom: 15px;margin-top: 15px;border-top-style: solid;border-top-width: 3pt;border-top-color: #4F81BC;"> </div>
        ...
        <div class="invoice-header-left">
           ...
           <p> <b> Contact Person: </b> ${User?.username}</p>
      ...
      invoiceData?.createdAt
       ...
       <p>   <b>   State Name :</b> ${User?.statename} </p>
...
Thanks You for your payment 
      ...
      if (local === 1) {
        ...
        return ` 
            <p> <b> Rs. ${(
            totalWithGST - amountWithoutGST.toFixed(2)
          ).toFixed(2)} </b>  <p>
          `;
      }
    })()} </td>
...
<tr>
              ...
              <b> TOTAL </b> </td>
               ...
               <td style="text-align: right;"  > 
              <b> Rs. ${parseFloat(totalWithGST.toFixed(2))} </b> </td>
...
export const userOrderPayment = async (req, res) => {
    ...
    try {
        ...
        const myorder = await orderModel.findOneAndUpdate(
            { _id: orderId },
            { razorpay_order_id: order.id },
            { new: true }
        );
        ...
        console.log(error);
        return res.status(500).send({
            message: `Error occurred during payment initiation: ${error.message}`,
            success: false,
            error,
        });
    }
...
const amountWithoutGST = totalAmount ;

await AddWalletPayment(payment.userId, 0, payment.note, amountWithoutGST);


    res.redirect(
      `${process.env.BACKWEB}all-payment`
    );
  } else {
    // await orderModel.findOneAndUpdate(
    //   { razorpay_order_id },
...
export const updateVendorProfileUser = async (req, res) => {
  ...
  try {
    ...
    const profileImg = req.files ? req.files.profile : undefined;
     ...
     let statetax = 0;
    const mystate = await zonesModel.findOne({name:statename});
   ...
   if (typeof password === 'string' && password.length > 0 &&
    typeof confirm_password === 'string' && confirm_password.length > 0) {
  const hashedPassword = await bcrypt.hash(password, 10);
  updateFields.password = hashedPassword;
}
...
export const getCategoriesWithProducts_nocache = async (req, res) => {
  try {
...
export const getCategoriesWithProducts = async (req, res) => {
  try {
    ...
    // 🔑 Unique cache key per location
    const cacheKey = `categories_products_${location || "all"}`;

    // 🔹 Check cache first
    const cachedData = cache.get(cacheKey);
    if (cachedData) {
      return res.status(200).send({
        message: "Categories and Products fetched successfully (from cache)",
        success: true,
        location,
        categoriesWithProducts: cachedData,
      });
    }
    ...
    // 🔹 Save to cache for 1 hour
    cache.set(cacheKey, filteredCategories);

    return res.status(200).send({
      message: "Categories and Products fetched successfully (from DB)",
      success: true,
      location,
      categoriesWithProducts: filteredCategories,
    });
